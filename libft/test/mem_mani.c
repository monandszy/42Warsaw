...
int test_memset(void)
{
    ...
    for (int i = 0; test_cases[i].n != 0 || test_cases[i].c != 0; ++i)
    {
        ...
        void *ft_ret = ft_memset(ft_buf, test_cases[i].c, test_cases[i].n);
        ...
        if (memcmp(ft_buf, std_buf, 20) != 0)
        {
            ...
            printf("Expected buffer modified by memset, Got buffer modified by ft_memset\n");
            return (1);
        }
    }
    return (0);
...
int test_memcpy(void)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        void *ft_ret = ft_memcpy(ft_dest, src, n);
...
int test_memmove(void)
{
    ...
    memcpy(ft_buf, initial, 27);
    memcpy(std_buf, initial, 27);
    void *ft_ret = ft_memmove(ft_buf + 5, ft_buf, 10);
    ...
    if (memcmp(ft_buf, std_buf, 27) != 0) {
        printf("FAIL: test_memmove | content, dest > src | dest: %s, src: %s, n: %d \n", ft_buf + 5, ft_buf, 10);
        printf("Expected: \"%s\", Got: \"%s\"\n", std_buf, ft_buf);
        return(1);
    }
