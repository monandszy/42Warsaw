...
void initalize_commands(t_data *data)
{
  ...
  while (i < data->cmd_count)
  {
    ...
    args = ft_strrchr(data->cmd[i].command, ' ');
    if(args)
    {
      name = ft_substr(data->cmd[i].command, 0, args - data->cmd[i].command);
      data->cmd[i].name = ft_strjoin("/", name);
      free(name);
      data->cmd[i].args = args + 1;
    }
    else
    {
      data->cmd[i].name = ft_strjoin("/", data->cmd[i].command);
      data->cmd[i].args = "";
    }
    if(!data->cmd[i].name)
      end(data, "malloc error\n");
